...
begin
  ...
  where r.client_id = p_client_id
    and upper(trim(coalesce(r.tariff_code, ''))) = upper(trim(v_code))
  ...
  if coalesce(p_enabled, false) = false then
    ...
    where client_id = p_client_id
      and upper(trim(coalesce(tariff_code, ''))) = upper(trim(v_code));
    return true;
  end if;

  if v_rule.id is not null then
    update public.client_tariff_rules
    set name = coalesce(v_product.nom, v_product.ref, 'Tarif standard'),
        tariff_code = v_code,
        service_level = null,
    ...
    insert into public.client_tariff_rules (
      client_id, name, tariff_code, service_level,
      ...
      p_client_id, coalesce(v_product.nom, v_product.ref, 'Tarif standard'), v_code, null,
...
begin
  ...
  if v_existing is not null then
    ...
    set type_remise = coalesce(nullif(trim(p_type_remise), ''), 'supp'),
        ...
        actif = coalesce(p_enabled, false)
    where id = v_existing;
  else
