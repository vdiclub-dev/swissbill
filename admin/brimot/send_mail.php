...
// Corps sans URL brute (le lien « Voir la facture en ligne » est ajouté à part)
$bodyDisplay = $body;
if ($viewUrl !== '') {
    $bodyDisplay = trim(preg_replace('/\n{3,}/u', "\n\n", str_replace($viewUrl, '', $body)));
}
...
// ─── HTML de l'email ──────────────────────────────────────────────────────────
$bodyHtml  = nl2br(htmlspecialchars($bodyDisplay, ENT_QUOTES, 'UTF-8'));

// Note piece jointe
$attachNote = '';
if ($hasAttach) {
    if ($pdfB64 !== '') {
        $attachNote = '<p style="margin-top:12px;padding:8px 12px;background:#f0f9ff;border-left:3px solid #0ea5e9;font-size:11px;color:#0369a1;">Le PDF de la facture est joint à cet email.</p>';
    } else {
        $attachNote = '<p style="margin-top:12px;padding:8px 12px;background:#f0f9ff;border-left:3px solid #0ea5e9;font-size:11px;color:#0369a1;">La facture (fichier HTML) est jointe à cet email. Ouvrez-la dans votre navigateur pour voir le QR-bill de paiement.</p>';
    }
}
...
if ($viewUrl !== '') {
    ...
    $viewUrlHtml = '<p style="margin:18px 0 0;">'
        . '<a href="' . $safeUrl . '" style="color:#2563eb;text-decoration:underline;font-weight:600;">Voir la facture en ligne</a>'
        . '</p>';
}
...
if ($hasAttach) {
    ...
    $mailBody .= quoted_printable_encode($bodyDisplay . $viewUrlText) . "\r\n\r\n";
