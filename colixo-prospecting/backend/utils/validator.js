// Validation centralisée — retourne { ok, errors }
...
function validateProspect(data) {
  ...
  const VALID_STATUTS = [
    'a_contacter','analyse_en_cours','pret_a_contacter','contact_envoye',
    'en_attente','repondu','relance_a_faire','rdv','opportunite','perdu'
  ];
  if (data.statut && !VALID_STATUTS.includes(data.statut)) {
    errors.push(`"statut" invalide. Valeurs acceptées : ${VALID_STATUTS.join(', ')}`);
  }
...
function validateTask(data) {
  ...
  if (data.due_date && isNaN(Date.parse(data.due_date))) {
    errors.push('"due_date" n\'est pas une date valide (format attendu: YYYY-MM-DD)');
  }
...
function validateReply(data) {
  ...
  if (!data?.raw_message?.trim()) errors.push('Le champ "raw_message" est requis');
  ...
  if (data.source && !VALID_SOURCES.includes(data.source)) {
    errors.push(`"source" invalide. Valeurs acceptées : ${VALID_SOURCES.join(', ')}`);
  }

  return { ok: errors.length === 0, errors };
}

module.exports = { validateProspect, validateTask, validateReply };
